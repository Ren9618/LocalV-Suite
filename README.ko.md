...
LocalV-Suite는 개인정보 보호를 중시하며 사용자의 PC에서 완전히 오프라인으로 실행되는 AI 방송 봇 및 어시스턴트 도구 모음입니다.
현재 메인 앱으로 YouTube Live 방송 댓글에 자동으로 반응하고 대화하는 AI 마스코트 **"Stream Companion"** 을 제공하고 있습니다.
...
*   **✨ 새 기능 및 관리:**
    ...
    *   **디버그 로그 UI:** 앱 동작 시각화. UI에서 "디버그 로그" 토글을 켜면 AI 워밍업이나 오류 상태 등 문제 해결 정보를 실 실시간으로 확인할 수 있습니다.
...
| 항목 | 환경 1 (Linux) | 환경 2 (Windows) | 환경 3 (Mac) |
|---|---|---|---|
| **OS** | Ubuntu 24.04 LTS (x86_64) | Windows 11 | macOS Tahoe 26.3 |
| **CPU** | AMD Ryzen 7 5800X (16스레드) | AMD Ryzen 7 5800X (16스레드) | Apple M1 |
| **GPU** | NVIDIA GeForce RTX 2080 (VRAM 8GB) | NVIDIA GeForce RTX 2080 (VRAM 8GB) | Apple M1 (8코어 GPU) |
| **RAM** | 48GB | 48GB | 16GB |
| **Node.js** | v25.x | v20.x 이상 | v20.x |
| **Python** | 3.10~3.14 (Voiceger 사용 시) | 3.10 (Voiceger 사용 시) | 3.9.7 |
...
현재 방송을 더욱 풍성하게 만들기 위해 다음과 같은 새로운 기능을 기획 및 검토 중입니다.

* **방송 화면 및 연출 강화**
  * **아바타 연동**: VTube Studio 등과 연동하여 AI의 감정에 따라 아바타의 표정이나 모션을 자동으로 전환합니다.
  * **효과음 (SE) 연동**: AI의 반응이나 특정 키워드에 맞춰 자동으로 효과음을 재생합니다.
  * **OBS 자동 제어**: OBS WebSocket API를 통해 AI 주도의 장면 전환 등 방송 운영을 보조합니다.

* **인터랙티브 및 시청자 참여 기능**
  * **멀티 플랫폼 지원**: YouTube 외에도 Twitch 등 다른 플랫폼의 댓글을 동시에 처리합니다.
  * **AI 주도 기획**: 댓글 창이 조용할 때 AI가 자동으로 설문조사나 미니 게임을 시작하여 대화 주제를 제공합니다.
  * **슈퍼챗 특별 연출**: 후원 금액에 따라 특별한 대사나 음성 톤으로 반응합니다.

* **지능형 기능 확장**
  * **음성 인식 (STT) 연동**: 시청자 댓글뿐만 아니라 스트리머의 마이크 음성을 인식하여 AI와 직접 대화(합방)할 수 있는 기능입니다.
  * **RAG (검색 증강 생성)**: 로컬 텍스트나 Wiki 데이터를 학습시켜 AI가 보다 전문적이고 문맥에 맞는 정확한 발언을 하도록 돕습니다.
  * **자동 하이라이트 생성**: 방송 종료 시 그날의 대화 내용을 요약하여 주요 장면을 텍스트로 출력하는 기능입니다.
