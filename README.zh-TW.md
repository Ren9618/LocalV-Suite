...
LocalV-Suite 是一款專注於隱私、完全離線運行於您的 PC 上的 AI 直播輔助機器人應用套件。
目前，主推的應用 **"Stream Companion"** 是一位能自動對 YouTube 直播評論做出反應並與觀眾對話的 AI 吉祥物。
...
|---|---|---|---|
| **作業系統** | Ubuntu 24.04 LTS (x86_64) | Windows 11 | macOS Tahoe 26.3 |
| **CPU** | AMD Ryzen 7 5800X (16執行緒) | AMD Ryzen 7 5800X (16執行緒) | Apple M1 |
| **GPU** | NVIDIA GeForce RTX 2080 (8GB顯存) | NVIDIA GeForce RTX 2080 (8GB顯存) | Apple M1 (8核心 GPU) |
| **記憶體** | 48GB | 48GB | 16GB |
| **Node.js** | v25.x | v20.x 以上 | v20.x |
| **Python** | 3.10〜3.14 (使用Voiceger時) | 3.10 (使用Voiceger時) | 3.9.7 |
...
## 🚀 發展路線與未來構想 (Roadmap / Future Features)

目前，為了讓直播更加生動有趣，我們正在構思並計劃加入以下新功能：

* **豐富直播畫面與表現力**
  * **虛擬形象聯動**: 結合 VTube Studio 等工具，根據 AI 的情緒自動切換立繪或虛擬形象的面部表情與動作。
  * **自動播放音效 (SE)**: 當 AI 吐槽或檢測到特定關鍵詞時，自動播放相對應的音效（如拍手聲、提示音等）。
  * **OBS 自動管控**: 透過 OBS WebSocket API 實現自動化導播，例如由 AI 發號施令進行場景切換。

* **強化互動與觀眾參與感**
  * **多平台兼容**: 除了 YouTube 外，也能同時接入 Twitch 等其他平台的聊天室，讓 AI 都能一一應對。
  * **AI 主導的互動環節**: 在聊天室較安靜時，AI 會主動發起投票調查或迷你小遊戲，活躍直播間氣氛。
  * **SuperChat 專屬特效**: 根據打賞金額的不同，AI 會做出特殊語音語調的回應或彈出專屬對話。

* **提升智能與輔助能力**
  * **語音辨識 (STT) 融合**: 不僅讀取觀眾評論，還能辨識實況主的麥克風聲音，實現與 AI 的即時語音對話與「聯動直播」。
  * **RAG (檢索增強生成)**: 將本地文本或維基資料餵給 AI，使其能說出更具專業性、更貼合上下文的準確發言。
  * **自動生成精華時刻**: 在直播結束時，由 AI 自動摘要當天的精彩互動，輸出可供發布或回顧的文本紀錄。
