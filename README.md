...
### 映像（字幕）の連携
1. アプリ右下の「🔗 OBS用URLをコピー」をクリックし、コピーしたURL（`http://localhost:25252/`）を使用します。
2. OBSで「ブラウザソース」を追加し、URLを貼り付けます（アプリから提供されるURLはすでに透明背景になっています）。
3. **🎨 字幕デザインのカスタマイズ**: アプリを一度起動すると `[ユーザーディレクトリ]\AppData\Roaming\local-v-streamlive-app\overlay.html` （Windowsの場合）が作成されます。このHTMLファイル内の CSS を直接書き換えることで、文字色や大きさ、フォントや透明背景などを完全にお好みのデザインに変更できます！
